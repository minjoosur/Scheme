...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            raise SchemeError('More arguments passed in than needed')
        elif len(formals) > len(vals):
            raise SchemeError('Not enough arguments passed in')
        else:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('TypeError when calling {0}'.format(self.name))
...
def do_and_form(expressions, env):
    ...
    # If we are at the end of an OR expression, eval as if it were a tail context.
    if expressions.second is nil:
        return scheme_eval(expressions.first, env, True)
    else:
        evaluated = scheme_eval(expressions.first, env, False)
...
def do_or_form(expressions, env):
    ...
    # If we are at the end of an AND subexpression, eval as if it were a tail context. 
    if expressions.second is nil:
        return scheme_eval(expressions.first, env, True)
    else:
        evaluated = scheme_eval(expressions.first, env, False)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            operator = scheme_eval(first, env)
            check_procedure(operator)
            result = scheme_apply(operator, rest.map(lambda x: scheme_eval(x, env)), env)
